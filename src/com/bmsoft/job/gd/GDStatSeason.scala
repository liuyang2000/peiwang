...
object GDStatSeason {
  def main(args: Array[String]): Unit = {
    val sc = new SparkContext(new SparkConf().setAppName("GDStatSeason")
    ...
    val stat = new GDStatImpl(hc, dt, DateFormatUtil.STAT_CYCLE_SEASON)
    val df = stat.statGDDetail.repartition(8)

    val jdbc = new GDStatJdbc()
    jdbc.insertGDStatList(df, dt, DateFormatUtil.STAT_CYCLE_SEASON)
  }
}