...
object GDStatMon {
  def main(args: Array[String]): Unit = {
    val sc = new SparkContext(new SparkConf().setAppName("GDStatMon")
    ...
    val stat = new GDStatImpl(hc, dt, DateFormatUtil.STAT_CYCLE_MON)
    val df1 = stat.statGDDetail.repartition(8)

    val jdbc = new GDStatJdbc()
    jdbc.insertGDStatList(df1, dt, DateFormatUtil.STAT_CYCLE_MON)
  }
}