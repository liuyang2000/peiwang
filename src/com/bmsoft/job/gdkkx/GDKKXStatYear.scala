 ...
 * @author mathsyang
 */
object GDKKXStatYear {
  def main(args: Array[String]): Unit = {
    val sc = new SparkContext(new SparkConf().setAppName("GDKKXStatYear")
    ...
    val stat = new GDKKXStatImpl(hc, dt, DateFormatUtil.STAT_CYCLE_YEAR)
    val jdbc = new GDKKXStatJdbc()
    val df1 = stat.statTZ.repartition(8).persist()
    jdbc.insertTZStatList(df1, dt, DateFormatUtil.STAT_CYCLE_YEAR)
    df1.unpersist()
    
    val df2 = stat.statGDKKX.repartition(8).persist()
    import hc.implicits._
    jdbc.updateDGKKX(df2.filter($"SSBZ" === "SS"), dt, DateFormatUtil.STAT_CYCLE_YEAR, "SS")
    jdbc.updateDGKKX(df2.filter($"SSBZ" === "BZ"), dt, DateFormatUtil.STAT_CYCLE_YEAR, "BZ")
    df2.unpersist()
  }
}