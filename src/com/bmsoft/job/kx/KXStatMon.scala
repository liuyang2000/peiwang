...
object KXStatMon {
  def main(args: Array[String]): Unit = {
    val sc = new SparkContext(new SparkConf().setAppName("KXStatMon")
    ...
    val stat = new KXStatImpl(hc, dt, DateFormatUtil.STAT_CYCLE_MON)
    ...
    jdbc.insertKXExcept(df2, dt, DateFormatUtil.STAT_CYCLE_MON)
    df2.unpersist()
    val df3 = stat.statSSExcept.repartition(8).persist()
    jdbc.insertSSKXExcept(df3, dt, DateFormatUtil.STAT_CYCLE_MON)
    df3.unpersist()
    val df4 = stat.statSSGatherSuccRate.repartition(8).persist()
    jdbc.insertKXSSGatherSucc(df4, dt, DateFormatUtil.STAT_CYCLE_MON)
    df4.unpersist()
    val df5 = stat.statBZExcept.repartition(8).persist()
    jdbc.insertBZKXExcept(df5, dt, DateFormatUtil.STAT_CYCLE_MON)
    df5.unpersist()
    val df6 = stat.statBZGatherSuccRate.repartition(8).persist()
    jdbc.insertKXBZGatherSucc(df6, dt, DateFormatUtil.STAT_CYCLE_MON)
    df6.unpersist()
    val df7 = stat.statSSClass.repartition(8).persist()
    jdbc.insertKXSSClass(df7, dt, DateFormatUtil.STAT_CYCLE_MON)
    df7.unpersist()
  }
}