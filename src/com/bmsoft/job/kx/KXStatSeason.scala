...
object KXStatSeason {
  def main(args: Array[String]): Unit = {
    val sc = new SparkContext(new SparkConf().setAppName("KXStatSeason")
    ...
    val stat = new KXStatImpl(hc, dt, DateFormatUtil.STAT_CYCLE_SEASON)
    ...
    jdbc.insertKXExcept(df2, dt, DateFormatUtil.STAT_CYCLE_SEASON)
    df2.unpersist()
    val df3 = stat.statSSExcept.repartition(8).persist()
    jdbc.insertSSKXExcept(df3, dt, DateFormatUtil.STAT_CYCLE_SEASON)
    df3.unpersist()
    val df4 = stat.statSSGatherSuccRate.repartition(8).persist()
    jdbc.insertKXSSGatherSucc(df4, dt, DateFormatUtil.STAT_CYCLE_SEASON)
    df4.unpersist()
    val df5 = stat.statBZExcept.repartition(8).persist()
    jdbc.insertBZKXExcept(df5, dt, DateFormatUtil.STAT_CYCLE_SEASON)
    df5.unpersist()
    val df6 = stat.statBZGatherSuccRate.repartition(8).persist()
    jdbc.insertKXBZGatherSucc(df6, dt, DateFormatUtil.STAT_CYCLE_SEASON)
    df6.unpersist()
    val df7 = stat.statSSClass.repartition(8).persist()
    jdbc.insertKXSSClass(df7, dt, DateFormatUtil.STAT_CYCLE_SEASON)
    df7.unpersist()
  }
}