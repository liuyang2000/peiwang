...
object PBStatHYear {
  def main(args: Array[String]): Unit = {
    val sc = new SparkContext(new SparkConf().setAppName("PBStatHYear")
      .set("spark.shuffle.reduceLocality.enabled", "false")
      .set("spark.eventLog.enabled", "false"))
    val hc = new HiveContext(sc)

    val dt = if (args.length == 0 || args(0) == null) DateFormatUtil.dateHY(DateFormatUtil.getCycleDate, 0)
    ...
    val stat = new PBStatImpl(hc, dt, DateFormatUtil.STAT_CYCLE_HYEAR)
    ...
    if (tabs.contains("2")) {
      ...
      jdbc.insertPBExcept(df2, dt, DateFormatUtil.STAT_CYCLE_HYEAR)
      df2.unpersist()
    }
    if (tabs.contains("4")) {
      val df4 = stat.statSSGatherSuccRate.repartition(8).persist()
      jdbc.insertPBSSGatherSucc(df4, dt, DateFormatUtil.STAT_CYCLE_HYEAR)
      df4.unpersist()
    }
    if (tabs.contains("6")) {
      val df6 = stat.statBZGatherSuccRate.repartition(8).persist()
      jdbc.insertPBBZGatherSucc(df6, dt, DateFormatUtil.STAT_CYCLE_HYEAR)
      df6.unpersist()
    }
    if (tabs.contains("3")) {
      val df3 = stat.statSSExcept.repartition(8).persist()
      jdbc.insertSSPBExcept(df3, dt, DateFormatUtil.STAT_CYCLE_HYEAR)
      df3.unpersist()
    }
    if (tabs.contains("5")) {
      val df5 = stat.statBZExcept.repartition(8).persist()
      jdbc.insertBZPBExcept(df5, dt, DateFormatUtil.STAT_CYCLE_HYEAR)
      df5.unpersist()
    }
//    if (tabs.contains("7")) {
//      val df7 = stat.statSSClass.repartition(8).persist()
//      jdbc.insertPBSSClass(df7, dt, DateFormatUtil.STAT_CYCLE_HYEAR)
//      df7.unpersist()
//    }
  }
}