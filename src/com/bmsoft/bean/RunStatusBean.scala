...
class RunStatusBean(val ID: Option[String],
    ...
    val MAXS_T :Option[Double]) extends Product with Serializable {
  ...
  override def canEqual(that: Any): Boolean = that.isInstanceOf[RunStatusBean]
}
